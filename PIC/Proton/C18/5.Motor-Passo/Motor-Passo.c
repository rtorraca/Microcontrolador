...
Descrição: Controle de motor de passo 28byj-48 de 5Vdc c/ driver circuito integrado ULN2003
			ao acionar botão placa PA3 roda inverso
			- na pasta projeto/ em otrher files diagrama motor
...
Ligações:
Pinos PIC/PROTO'n   ULN2003			Motor cor fio
RB4-----------------4	 13---------AM	
RB5-----------------3  	 14---------Rs
RB6-----------------2  	 15---------Az 
RB7-----------------1	 16---------Lr	
...
Notas: Notas: usando programa acima endereço 0x1000 p/ compatibilidade c/ BootLoader USB
...
void step_clock_wise (void);
void step_counter_clock_wise (void);
...
void main (void)
{
	...
	while (1)
	{
		...
		// Se chave estiver acionar roda anti horário
		if ( PORTAbits.RA3 == 1 )
			step_clock_wise ();
		else
			step_counter_clock_wise();
...
void step_clock_wise (void)
{
	...
	passo <<=1;
	if (passo == 0x10) passo = 1; else if (passo == 0) passo = 8; 
	PORTB = (passo << 4);
